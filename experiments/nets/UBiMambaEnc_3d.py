...
class UMambaEnc(UNet):
    def __init__(
        ...
        super().__init__(input_size, input_channels, n_stages, features_per_stage, strides)
    ...
    @classmethod
    def from_plan(cls, plan, input_channels, output_channels):
        jplan = plan["configurations"]["3d_fullres"]
        num_stages = len(jplan["conv_kernel_sizes"])
        model = cls(
            input_channels=input_channels,
            n_stages=num_stages,
            features_per_stage=[
                min(
                    jplan["UNet_base_num_features"] * 2**i,
                    jplan["unet_max_num_features"],
                )
                for i in range(num_stages)
            ],
            conv_op=nn.Conv3d,
            kernel_sizes=jplan["conv_kernel_sizes"],
            strides=jplan["pool_op_kernel_sizes"],
            num_classes=output_channels,
            deep_supervision=False,
            input_size=jplan["patch_size"],
            conv_bias=True,
            norm_op=nn.InstanceNorm3d,
            norm_op_kwargs={"eps": 1e-5, "affine": True},
            nonlin=nn.LeakyReLU,
            nonlin_kwargs={"inplace": True},
            n_conv_per_stage=jplan["n_conv_per_stage_encoder"],
            n_conv_per_stage_decoder=jplan["n_conv_per_stage_decoder"],
        )
        return model
