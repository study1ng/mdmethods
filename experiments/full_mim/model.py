...
class UNetWithHead(nn.Module):
    def __init__(
        ...
        self.output_size = unet.output_size
        self.head = head_fn(
...
class UNetWithMIM(L.LightningModule):
    def __init__(
        self,
        unet: UNet,
        patch_channel,
        ...
        if self.weights is None:
            self.weights = [2.**i for i in range(self.unet.n_stages)]
        elif isinstance(self.weights, float):
            self.weights = [self.weights**i for i in range(self.unet.n_stages)]
        self.weights = np.array(self.weights)
        self.weights /= self.weights.sum()
        self.weights = torch.Tensor(self.weights)
        self.register_buffer("loss_weights", self.weights, persistent=True)
        # self.mask_token = nn.Parameter(
        #     torch.zeros(1, self.unet.input_channel, 1, 1, 1)
        # )
